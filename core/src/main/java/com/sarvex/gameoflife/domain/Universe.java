...
public class Universe {
      ...
      for (int x = 0; x < maxColumn; x++) { // From left to right
        ...
        if (currentCell == Cell.LIVE_CELL) { // If cell is currently LIVE
          if ((neighbourCount == 2) || (neighbourCount == 3)) {
            nextCell = Cell.LIVE_CELL; // Stay LIVE if 2 or 3 neighbours are LIVE
          } else {
            nextCell = Cell.DEAD_CELL; // Else, become DEAD due to underpopulation or overcrowding
          }
        } else { // If cell is currently DEAD
          if (neighbourCount == 3) {
            nextCell = Cell.LIVE_CELL; // Become LIVE if 3 neighbours are LIVE
          } else {
            nextCell = Cell.DEAD_CELL; // Else, stay DEAD
          }
        }
  ...
  public void setLiveCellAt(final int row, final int column) {
    this.currentGridContent.setCellAt(column, row, Cell.LIVE_CELL);
  }

  // Set cell at given coordinate to DEAD
  public void setDeadCellAt(final int row, final int column) {
    this.currentGridContent.setCellAt(column, row, Cell.DEAD_CELL);
  }
