...
public class Grid {
  ...
  private Cell[][] anArrayOfDeadCells(final int rows, final int columns) {
    ...
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < columns; j++) {
        deadCells[i][j] = Cell.DEAD_CELL; // Then set each one's status to DEAD
      }
    }
    return deadCells;
  }
  ...
  // However, if validation later becomes complex, a separate function is good practice
